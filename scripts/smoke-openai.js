...
import { DOCKETBOUND_MODEL, responseText, runToolLoop } from "../src/lib/openai-client.js";
...
const { response, toolCalls } = await runToolLoop({
  model: DOCKETBOUND_MODEL,
  instructions: "You are the single DocketBound orchestrator. Use the fixture tool before stating any docket fact. Return a one-sentence confirmation without adding citations or facts not returned by the tool.",
...
console.log(JSON.stringify({ ok: true, model: DOCKETBOUND_MODEL, toolCalls, responseId: response.id }, null, 2));
